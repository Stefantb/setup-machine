...
# Sets up on a new machine with Pop!_OS 24.04
...
install_bitwarden() {
    func_begin "Installing Bitwarden"
...
install_neovim() {
    ...
    pushd ~/dev/local-tools
    git clone https://github.com/neovim/neovim.git --branch release-0.12
...
install_vivaldi() {
    ...
    vivaldi_version="vivaldi-stable_7.9.3970.45-1_amd64.deb"
    pushd ~
    wget https://downloads.vivaldi.com/stable/${vivaldi_version}
...
read -rsn1 -p "Start Bitwarden and press any key to continue..." < /dev/tty
